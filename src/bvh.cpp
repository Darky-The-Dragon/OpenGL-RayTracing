...
static int build_recursive(std::vector<BVHNode> &nodes, const std::vector<CPU_Triangle> &tris,
    ...
    int leftIdx = build_recursive(nodes, tris, refs, begin, mid, leafMax);
    int rightIdx = build_recursive(nodes, tris, refs, mid, end, leafMax);
...
std::vector<BVHNode> build_bvh(std::vector<CPU_Triangle> &tris) {
    ...
    for (size_t i = 0; i < tris.size(); ++i) {
        refs[i].triIndex = (int) i;
        refs[i].c = tri_centroid(tris[i]);
    }
    nodes.reserve(tris.size() * 2);
    build_recursive(nodes, tris, refs, 0, (int) refs.size(), 8);

    // IMPORTANT: reorder triangles to match leaf ranges for better locality.
    // Create a remap array from refs[leaf ranges]
    std::vector<CPU_Triangle> remapped;
    remapped.reserve(tris.size());

    // We walk leaves and gather triangles in order.
    // Simple DFS stack:
    std::vector<int> stack;
    stack.push_back(0);
    std::vector<int> leafRanges; // we will replace nodes[first,count] to new indices
    while (!stack.empty()) {
        int n = stack.back();
        stack.pop_back();
        ...
        if (node.isLeaf()) {
            ...
            // Overwrite first to point into remapped order base.
            int base = (int) remapped.size() - node.count;
            //const_cast<BVHNode &>(node); // (silence)
            // store back
            ((BVHNode &) nodes[n]).first = base;
        } else {
...
void upload_bvh_tbos(const std::vector<BVHNode> &nodes,
                     const std::vector<CPU_Triangle> &tris,
                     GLuint &outNodeTex, GLuint &outNodeBuf,
                     GLuint &outTriTex, GLuint &outTriBuf) {
    ...
    for (auto &n: nodes) {
        ...
        nodeData.push_back((float) n.left);
        nodeData.push_back(n.bmax.x);
        nodeData.push_back(n.bmax.y);
        nodeData.push_back(n.bmax.z);
        nodeData.push_back((float) n.right);
        nodeData.push_back((float) n.first);
        nodeData.push_back((float) n.count);
...
void gather_model_triangles(const Model &model,
    ...
    for (const auto &mesh: model.meshes) {
        ...
        for (size_t k = 0; k + 2 < I.size(); k += 3) {
            glm::vec3 p0 = glm::vec3(M * glm::vec4(V[I[k]].Position, 1.0f));
            glm::vec3 p1 = glm::vec3(M * glm::vec4(V[I[k + 1]].Position, 1.0f));
            glm::vec3 p2 = glm::vec3(M * glm::vec4(V[I[k + 2]].Position, 1.0f));
            CPU_Triangle T;
            T.v0 = p0;
            T.e1 = p1 - p0;
            T.e2 = p2 - p0;
